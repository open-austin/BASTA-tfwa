...
namespace TenantFile.Api.Controllers
{
    public class SmsController : Controller
    {
        ...
        {
            var phone = await context.Phones.AsQueryable()
                            .Include(p => p.Tenants)
                            .FirstAsync(x => x.PhoneNumber == data.from)!;

            var property = await context.Properties.Include(p => p.Address)
                        .AsQueryable()
                        .FirstAsync(p => p.Name == data.propertyName);
        ...
        [HttpPost("/api/captureMoldData")]
        public async Task<IActionResult> CaptureMoldData([FromBody] MoldFlowData data)
        {
            var phone = await context.Phones.AsQueryable()
                 .Include(p => p.Tenants)
                 .Include(p => p.Images)
            ...
            foreach (var image in sentImages)
            {
        ...
        [HttpPost("/api/mold")]
        public async Task<IActionResult> MoldWebhook()
        {
            var request = await JsonSerializer.DeserializeAsync<IDictionary<String, JsonElement>>(Request.Body);
            ...
            await context.SaveChangesAsync();
            var imageIds = string.Join(',', phone.Images.Select(i => i.Id.ToString()));
            if (newPhone == true)
            {
